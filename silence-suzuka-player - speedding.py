...
class PlaylistLoaderThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            # Existing Bilibili space playlist URL cleaning
            if self.kind == 'bilibili' and 'space.bilibili.com' in self.url:
                try:
        ...
        try:
            if info is None:
                self.itemsReady.emit([])
                return
            
            if isinstance(info, dict) and info.get('entries'):
                # Get the playlist title - this is the key fix
                playlist_title = info.get('title') or info.get('playlist_title') or self.url
                
                # For Bilibili space playlists, also try to get uploader name if title is generic
                if self.kind == 'bilibili' and 'space.bilibili.com' in self.url:
                    uploader = info.get('uploader') or info.get('channel') or ''
                    if uploader and (not playlist_title or playlist_title == self.url):
                        playlist_title = f"{uploader}'s Playlist"
                    elif uploader and playlist_title:
                        # If we have both, use the actual playlist title but you could combine them
                        pass  # playlist_title is already set correctly
                ...
                for entry in entries:
                    ...
                    # Create item with proper playlist metadata
                    item = {
                        'title': title,
                        'url': u,
                        'type': self.kind,
                        'playlist': playlist_title,  # Use the fetched playlist title
                        'playlist_key': info.get('id') or info.get('webpage_url') or self.url
                    }
        ...
        while not self._should_stop:
            ...
            try:
                ...
                if title and title != url:
                    ...
                    # For Bilibili, sometimes the title might be in a different field
                    if kind == 'bilibili' and isinstance(info, dict):
                        # Try alternative title fields
                        alt_title = info.get('description', '').split('\n')[0] if info.get('description') else None
                        if alt_title and len(alt_title) < 200:  # Reasonable title length
                            self.titleResolved.emit(url, alt_title)
...
class MediaPlayer(QMainWindow):
    ...
    def _on_title_resolved(self, url: str, title: str):
        try:
            ...
            for item in self.playlist:
                if item.get('url') == url:
                    # Only update if the title is actually different and meaningful
                    old_title = item.get('title', '')
                    if title and title != url and title != old_title:
                        item['title'] = title
                        updated = True
                        logger.debug(f"[Title] Resolved: {url[:50]}... -> {title}")
                    break
    ...
    def add_link_dialog(self):
        ...
        # ... (local file handling stays the same) ...
        ...
        if will_try_playlist:
            ...
            # Single network item - add with temporary title
            display = Path(url_in).name or url_in
            
            # For Bilibili single videos, use a loading placeholder
            if t == 'bilibili':
            ...
            # Trigger title resolution for both YouTube and Bilibili
            if t in ('youtube', 'bilibili'):
...
# Add this debug version to your _remove_all_in_group method:
    ...
    def _on_playlist_items_ready(self, items: list):
        ...
        # Get playlist name from the first item
        playlist_name = new_items[0].get('playlist', 'playlist') if new_items else 'playlist'
        
        # Add all new items at once
        start_index = len(self.playlist)
        self.playlist.extend(new_items)
        
        # Trigger title resolution for items that need it (YouTube and Bilibili)
        for i, item in enumerate(new_items):
            if item.get('type') in ('youtube', 'bilibili'):
                # Check if title needs resolution (is a URL or generic)
                title = item.get('title', '')
                url = item.get('url', '')
                
                # If title is empty, matches URL, or looks like a video ID, resolve it
                if not title or title == url or title.startswith('BV') or title.startswith('av'):
                    # Use the parallel title resolution system
                    self._resolve_title_parallel(url, item.get('type'))
        ...
        self.status.showMessage(f"Added {len(new_items)} entries from '{playlist_name}'", 5000)
