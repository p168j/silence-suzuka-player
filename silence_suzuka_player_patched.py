...
def get_user_data_dir(app_name: str = "SilenceSuzuka"):
    """Return a user-writable app data directory, creating it if needed."""
    from pathlib import Path
    import os, sys
...
class NetworkErrorHandler:
    ...
    def _warn(title, text):
        """Show a warning only if we're on the GUI thread; otherwise log."""
        try:
            import threading, logging
            from PySide6.QtWidgets import QMessageBox
            from PySide6.QtGui import QGuiApplication
            if threading.current_thread() is threading.main_thread() and QGuiApplication.instance() is not None:
                parent = QGuiApplication.activeWindow()
                try:
                    NetworkErrorHandler._warn(parent, title, text)
                except Exception:
                    logging.warning(f"{title}: {text}")
            else:
                logging.warning(f"{title}: {text}")
        except Exception as e:
            try:
                import logging as _logging
                _logging.warning(f"Failed to show warning '{title}': {e}")
            except Exception:
                pass
    @staticmethod
    def safe_ytdl_call(func, url, *args, **kwargs):
        ...
        try:
            ...
            NetworkErrorHandler._warn(None, "Connection Timeout", 
            ...
            if any(phrase in error_output.lower() for phrase in ['private video', 'video unavailable', 'this video is unavailable']):
                NetworkErrorHandler._warn(None, "Video Unavailable", 
    ...
    def show_friendly_error(error, url, operation="load"):
        ...
        if 'timeout' in error_str:
            NetworkErrorHandler._warn(None, "Timeout", 
    ...
    def run(self):
        try:
            ...
            if items is None:
                items = []
            total = len(items)
            self.totalKnown.emit(total)
            if total == 0 or self.isInterruptionRequested():
                self.finished.emit()
                return
            count = 0
...
def fetch_playlist_with_progress(player, url: str, title: str = "Fetching playlist", chunk_size: int = 200):
    ...
    def on_total(total):
        try:
            # If total is unknown/zero, keep indeterminate
            if total and total > 0:
                progress_dialog.setMaximum(total)
                progress_dialog.setValue(0)
        except Exception:
            pass
    thread.totalKnown.connect(on_total)
    thread.chunkReady.connect(on_chunk)
    thread.error.connect(on_error)
    thread.finished.connect(on_finished)
    progress_dialog.canceled.connect(lambda: (thread.requestInterruption(), thread.wait()))
...
class PlaylistManagerDialog(QDialog):
    ...
    def _save_playlists(self):
        """Save the playlist data back to the JSON file (both legacy path and user app-data)."""
        try:
            # Legacy path (script directory) for backward compatibility
            legacy_file = Path(__file__).parent / 'playlists_v2.json'
            legacy_file.parent.mkdir(exist_ok=True)
            # New user data path
            user_dir = get_user_data_dir()
            user_file = user_dir / 'playlists_v2.json'
            # Write atomically where possible
            data_str = json.dumps(self.saved_playlists, indent=2, ensure_ascii=False)
            for target in (user_file, legacy_file):
